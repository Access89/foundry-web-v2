...
const NewUseCases = () => {
  ...
  return (
    <div className=" mx-[1.5rem] md:mx-[5rem] lg:mx-[7rem] py-8">
      <h3 className="text-medium md:text-6xl  font-semibold mb-6 w-[90%] xl:w-[70%]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 my-[7rem]">
        {use_cases.map((uc) => (
          <Link
            key={uc.title}
            to={uc.link}
          ...
          >
            ...
            />
            {/* Gradient overlay */}
            <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent/5 to-black/90" />

            {/* Bottom title overlay */}
            <div className="group-hover:opacity-0 absolute bottom-6 left-8 w-full text-white text-2xl font-semibold transition-all duration-300">
              {uc.title}
            </div>
